...
if git add data/data_gouv_search_collections.json && git commit -m "Daily collections dictionnary update on $(date '+%d-%m-%Y %H:%M:%S')" 2>&1 | tee -a "$LOG_FILE"; then
...
if git push origin main 2>&1 | tee -a "$LOG_FILE"; then
    log "Changes pushed successfully on $(date '+%d-%m-%Y %H:%M:%S')"
else
